...
void format_output(vector<product> bag1, vector<product> bag2)
{
    int sum_weight = 0;
    int sum_price = 0;
    ...
    {
        sum_weight += bag1[i].weight;
        sum_price += bag1[i].price;
    }

    for (size_t i = 0; i < bag2.size(); i++)
    {
        sum_weight += bag2[i].weight;
        sum_price += bag2[i].price;
    }
    cout << sum_weight << " " << sum_price << " 1" << endl;
...
#pragma omp task shared(children_products)
            {
                vector<product> child_products = global_min(new_possible_products, new_selected_products, max_weight);
...
#pragma omp single
        {
            vector<product> bag1 = global_min(all_products, {}, weight_max);
        }
    }
